...
resource "aws_instance" "monitor_hub" {
  ...
  connection {
    user        = "ubuntu"
    host = self.public_ip
    private_key = "${file("/home/haitham/.ssh/Haitham-IAM-keypair.pem")}"
...
# # Install docker in the ubuntu
  ...
  #     "git clone https://github.com/ghalwash/icon-prometheus-exporter",
  #     "cd icon_prometheus_exporter/icon_prometheus_exporter/"
  #   ]
  # }
}
...
#   ip               = aws_eip_association.elastic_ips_association.public_ip
#   user             = "ubuntu"
#   private_key_path = var.private_key_path
#   playbook_file_path = "${path.module}/ansible/main.yml"
#   playbook_vars = {
#     ansible_host = var.root_domain_name == "" ? aws_eip.elastic_ips_for_instances.public_ip : join("", aws_route53_record.www.*.fqdn)
#   }
